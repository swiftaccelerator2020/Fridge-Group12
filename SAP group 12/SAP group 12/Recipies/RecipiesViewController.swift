...
extension RecipiesViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let requiredIngredients = self.listOfRecipes[indexPath.row].ingredients
        var numberOfPresentIngredients = 0
        for item in requiredIngredients {
            for presentItem in searchFoodItems {
                if removeWhitespaceAndHyphen(item) == removeWhitespaceAndHyphen(presentItem) {
                    numberOfPresentIngredients += 1
                    break
                }
            }
        }
        
        cell.ingredientsFullView.progress = Float(numberOfPresentIngredients) / Float(requiredIngredients.count)
